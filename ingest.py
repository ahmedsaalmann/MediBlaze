...
def main():
    ...
    print("Connecting to index and uploading...")
    ...
    for i in range(0, total, batch_size):
        batch = chunks[i:i + batch_size]
        print(f"Uploading batch {i//batch_size+1}: {i+1}-{i+len(batch)} of {total}")
        try:
            vectorstore.add_documents(documents=batch)
            uploaded += len(batch)
            print(f"OK. Total: {uploaded}/{total}")
            time.sleep(2)
        except Exception as e:
            print(f"Batch failed: {e}. Retrying after 10s...")
            time.sleep(10)
            try:
                vectorstore.add_documents(documents=batch)
                uploaded += len(batch)
                print(f"Retry OK. Total: {uploaded}/{total}")
            except Exception as e2:
                print(f"Retry failed: {e2}. Skipping batch.")

    print(f"Done! Uploaded {uploaded}/{total} chunks.")
